// ModbusReader - MBTCPPROTOCOL - A library for communication using Modbus/TCP from node.js.
...
function ModbusReader(){
	...
	// scouting specific configurations
	self.request = undefined;
	self.connection_name = undefined;
	self.intervalFunction = undefined;
	self.tags = undefined;
	self.variables = [];
	self.writeValues = undefined;
	self.mqttClient = undefined;
}

////*********************** [scouting specific functions] ***************************////


ModbusReader.prototype.configRequest = function (message, client) {
	var self = this;
	self.mqttClient = client;
	self.request = message;
	self.connection_name = self.request.config.controller.connection_name;
	self.tags = self.request.tags;
	for (var i = self.request.tags.length - 1; i >= 0; i--) {
		var block = self.request.tags[i];
		self.variables[block.name] = block.address;
	}
	self.writeValues = (self.request.values !== 'undefined') ? self.request.values : undefined;
}

ModbusReader.prototype.getConfigRequest = function () {
	var self = this;
	return self.request;
}


ModbusReader.prototype.addTags = function () {
	var self = this;
	self.setTranslationCB((tag) => {
		return self.variables[tag];
	});
	self.addItems(Object.keys(self.variables));
}


ModbusReader.prototype.init = function () {
	var self = this;
	self.initiateConnection({
		port: self.request.config.controller.port,
		host: self.request.config.controller.host,
		defaultID: 1,
		RTU: false
	});
}
...
ModbusReader.prototype.poll = function () {
	var self = this;
	self.intervalFunction = setInterval(() => {
		self.readAllItems();
	}, self.request.config.pullingInterval);
}

ModbusReader.prototype.write = function () {
	var self = this;
	setTimeout(() => {
		self.writeItems(Object.keys(self.variables), self.writeValues);
	}, 1000);
}

ModbusReader.prototype.shutdown = function () {
	var self = this;
...
ModbusReader.prototype.setTranslationCB = function(cb) {
	var self = this;
...
ModbusReader.prototype.initiateConnection = function (cParam, callback) {
	var self = this;
...
ModbusReader.prototype.dropConnection = function (callback) {
	var self = this;
...
ModbusReader.prototype.connectNow = function(cParam, suppressCallback) { // TODO - implement or remove suppressCallback
	var self = this;
	// Don't re-trigger.
...
ModbusReader.prototype.connectError = function(e) {
	var self = this;
...
ModbusReader.prototype.readWriteError = function(e) {
	var self = this;
...
ModbusReader.prototype.packetTimeout = function(packetType, packetSeqNum) {
	var self = this;
...
ModbusReader.prototype.onTCPConnect = function() {
	var self = this;
...
ModbusReader.prototype.writeItems = function(arg, value, cb) {
	var self = this;
	var i;
...
ModbusReader.prototype.findItem = function(useraddr) {
	var self = this;
	var i;
...
ModbusReader.prototype.addItems = function(arg) {
	var self = this;
	self.addRemoveArray.push({arg: arg, action: 'add'});
}

ModbusReader.prototype.addItemsNow = function(arg) {
	var self = this;
	var i;
...
ModbusReader.prototype.removeItems = function(arg) {
	var self = this;
	self.addRemoveArray.push({arg : arg, action: 'remove'});
}

ModbusReader.prototype.removeItemsNow = function(arg) {
	var self = this;
	var i;
...
ModbusReader.prototype.readAllItems = function() {
	var self = this;
	var i;
	...
	if (typeof arg === "function") {
		self.readDoneCallback = (anythingBad, vars) => {
			if (anythingBad) {
				console.log("##=> SOMETHING WENT WRONG READING VALUES!!!!");
			}
			var now = new Date();
			console.log("##=> done pulling in time : " + now.getMinutes() + ':' + now.getSeconds() + ':' + now.getMilliseconds());
			// publish items
			self.mqttClient.publish("devices/modbus/" + self.connection_name + "/out", JSON.stringify(vars));
		};
	} else {
...
ModbusReader.prototype.isWaiting = function() {
	var self = this;
	return (self.isReading() || self.isWriting());
}

ModbusReader.prototype.isReading = function() {
	var self = this;
	var i;
...
ModbusReader.prototype.isWriting = function() {
	var self = this;
	var i;
	// Walk through the array and if any packets are marked as sent, it means we haven't received our final confirmation.
...
ModbusReader.prototype.clearReadPacketTimeouts = function() {
	var self = this;
...
ModbusReader.prototype.clearWritePacketTimeouts = function() {
	var self = this;
...
ModbusReader.prototype.prepareWritePacket = function() {
	var self = this;
...
ModbusReader.prototype.prepareReadPacket = function() {
	var self = this;
...
ModbusReader.prototype.sendReadPacket = function() {
	var self = this;
...
ModbusReader.prototype.sendWritePacket = function() {
	var self = this;
...
ModbusReader.prototype.isOptimizableArea = function(area) {
	var self = this;
...
ModbusReader.prototype.onResponse = function(data) {
	var self = this;
...
ModbusReader.prototype.findReadIndexOfSeqNum = function(seqNum) {
	var self = this;
	var packetCounter;
...
ModbusReader.prototype.findWriteIndexOfSeqNum = function(seqNum) {
	var self = this;
	var packetCounter;
...
ModbusReader.prototype.writeResponse = function(data, foundSeqNum) {
	var self = this;
...
ModbusReader.prototype.readResponse = function(data, foundSeqNum) {
	var self = this;
...
ModbusReader.prototype.onClientDisconnect = function() {
	var self = this;
...
ModbusReader.prototype.onClientClose = function() {
	var self = this;
    // clean up the connection now the socket has closed
//	self.connectionCleanup();
...
ModbusReader.prototype.connectionReset = function() {
	var self = this;
	self.isoConnectionState = 0;
...
ModbusReader.prototype.resetNow = function() {
	var self = this;
	self.isoConnectionState = 0;
	self.isoclient.end();
	self.isoclient.destroy();
...
ModbusReader.prototype.connectionCleanup = function() {
	var self = this;
	self.isoConnectionState = 0;
